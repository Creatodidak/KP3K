package id.creatodidak.kp3k.pamatwil
...
import id.creatodidak.kp3k.pimpinan.AddAtensi
import id.creatodidak.kp3k.pimpinan.DashboardPimpinan
import id.creatodidak.kp3k.pimpinan.PanggilanVideo
import id.creatodidak.kp3k.pimpinan.PetaLahanPimpinan
import kotlinx.coroutines.launch
import androidx.core.content.edit
import id.creatodidak.kp3k.helper.formatDuaDesimalKoma
import id.creatodidak.kp3k.pimpinan.MonitoringPimpinan

class DashboardPamatwil : AppCompatActivity() {
    private lateinit var sh: SharedPreferences
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_dashboard_pamatwil)

        sh = getSharedPreferences("session", MODE_PRIVATE)

        val jabatan = sh.getString("jabatan", "")
        val penugasan = sh.getString("penugasan", "")
        ...
        changeAccount.setOnClickListener {
            AlertDialog.Builder(this)
                ...
                .setPositiveButton("Ya") { dialog, _ ->
                    dialog.dismiss()
                    if (role.equals("PAMATWIL")) {
                        with(sh.edit()) {
                            putString("role", "PIMPINAN")
                            apply()
                        }

                        val i = Intent(this, DashboardPimpinan::class.java)
                        startActivity(i)
                        finish()
                    }
                }
        ...
        val tvPenugasan = findViewById<TextView>(R.id.tvPenugasan)
        tvPenugasan.text = penugasan
    ...
    private suspend fun loadData() {
        ...
        try {
            ...
            val res = Client.retrofit.create(DataPimpinan::class.java).getBasicWilayah(kabupaten!!)
